...
const maxHeight = ref('max-h-[500px]');

const expandedCards = ref<Set<string>>(new Set());

const handleClickMore = (cardId: string) => {
  // Toggle max height for the card content
  ...
  <div class="grid h-auto grid-cols-1 gap-4 md:grid-cols-2">
    ...
    <div>
      <Card>
        ...
        <CardContent
          :class="expandedCards.has('ticket') ? 'max-h-full' : maxHeight"

          class="scroll-area overflow-hidden overflow-y-auto"
        >
          <div class="grid grid-cols-1 gap-4">
            <div v-for="ticket in tickets" :key="ticket.TKT_ID" class="rounded-lg bg-slate-50 p-4 dark:bg-slate-800/50">
              ...
              <div class="mt-2 flex justify-between text-sm text-slate-600 dark:text-slate-400">
                ...
                <span class="tabular-nums">{{ parseInt(ticket.total_registrations) - parseInt(ticket.total_checkins) }}
                  left</span>
              </div>
            </div>
          </div>
        </CardContent>
        <CardFooter v-if="tickets.length > 5" class="flex justify-end">
          <Button variant="outline" size="sm" @click="handleClickMore('ticket')">
            {{ expandedCards.has('ticket') ? 'Show Less' : 'Show More' }}
          </Button>
        </CardFooter>
      </Card>
    </div>
    ...
    <div v-if="custom && custom?.length" class="col-span-1 grid gap-4">
      <div class="space-y-4">
        <div v-for="question in custom" :key="question.info.QST_ID">
          <Card class="h-auto">
            <CardHeader>
              <CardTitle>Check-in Status by {{ question.info.QST_display_text }}</CardTitle>
            </CardHeader>
            <CardContent :class="maxHeight" class="scroll-area overflow-hidden overflow-y-auto">
              <div class="grid grid-cols-1 gap-4">
                <div
