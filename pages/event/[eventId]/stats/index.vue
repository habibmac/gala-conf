...
// Helper function to determine if a field is likely a country code field
const isCountryField = (fieldLabel: string): boolean => {
  const countryFieldNames = ['nationality', 'country', 'nation', 'origin'];
  return countryFieldNames.some(name => fieldLabel.toLowerCase().includes(name));
};

const valueFormatter = (tick: number | Date) =>
  typeof tick === 'number' ? formatThousands(tick) : '';

// Helper function to transform custom field data for display
const transformCustomFieldData = (field: any) => {
  const isCountryData = isCountryField(field.label);

  return field.values.map((v: CustomFieldChartItem) => {
    let displayValue = v.value || 'Not specified';

    // If it's a country field, try to convert country codes to full names
    if (isCountryData && v.value && v.value.length === 2) {
      const countryName = getCountryName(v.value.toUpperCase());
      displayValue = countryName !== v.value.toUpperCase() ? countryName : v.value;
    }
...
// Helper function to get display value for custom field legend
const getCustomFieldDisplayValue = (field: any, value: string) => {
  if (!value)
    return 'Not specified';

  const isCountryData = isCountryField(field.label);

  if (isCountryData && value.length === 2) {
    const countryName = getCountryName(value.toUpperCase());
    return countryName !== value.toUpperCase() ? countryName : value;
  }
...
const downloadCustomFieldData = (field: any) => {
  // Check if this field is likely a country field
  const isCountryData = isCountryField(field.label);

  // Create CSV content with proper escaping
  const csvRows = [
    ['Value', 'Count'], // Header row
    ...field.values.map((v: CustomFieldChartItem) => {
      ...
      return [escapeCsvValue(displayValue), escapeCsvValue(v.count)];
    }),
  ];
      ...
      <section v-if="customFieldStats && Object.keys(customFieldStats).length > 0" class="mb-8">
        ...
        <div v-else class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
          <Card v-for="(field, key) in customFieldStats" :key="key">
            ...
            <CardContent>
              <DonutChart
                :data="transformCustomFieldData(field)"
                index="name"
                category="total"
                :value-formatter="valueFormatter"
                class="h-48"
              ...
              <!-- Compact legend -->
              <div
                class="mt-4 max-h-32 space-y-1 overflow-y-auto [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-gray-300 dark:[&::-webkit-scrollbar-thumb]:bg-slate-500 [&::-webkit-scrollbar-track]:rounded-full [&::-webkit-scrollbar-track]:bg-gray-100 dark:[&::-webkit-scrollbar-track]:bg-slate-700 [&::-webkit-scrollbar]:w-1"
              >
                <div
                ...
                >
                  <span class="truncate text-muted-foreground">
                    {{ getCustomFieldDisplayValue(field, valueLegend.value) }}
                  </span>
                  <Badge variant="secondary" class="ml-2 rounded-full text-xs">
